...
const Scene = () => {
  ...
  return (
    <div   className='absolute bg-set p-[1rem]  max-h-[600px] border   bg-[#333]  rounded-md -top-80  right-20 w-[300px] overflow-x-hidden duration-500'>
      <div  className='mb-3'>
        {menu ? (
          <div  className='flex items-center justify-between'>
            <div className='cursor-pointer' onClick={() => backMenu()}>
              <svg
      ...
      <div >
        {sets.map((set,idx) => (
          <div  className='mb-3 cursor-pointer hover:opacity-60 duration-300 border rounded-md' key={idx} onClick={menu ? ( () => {} ) : ( () => handleDetails(set.scenes))}>
            <img
              src={set.img}
              alt='image'
            />
